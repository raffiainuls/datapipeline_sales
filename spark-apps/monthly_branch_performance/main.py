...
from monthly_branch_performance.sql import process_monthly_branch_performance
...
def main():
    spark = SparkSession.builder \
        .appName("monthly_branch_performance") \
        .getOrCreate()
    ...
    print("Execution monthly_branch_performance")
    monthly_branch_performance = process_monthly_branch_performance(spark)
    monthly_branch_performance.printSchema()
    monthly_branch_performance.show(truncate=False, n=20)

    writer = ch_writer()
    writer.write_to_clickhouse(monthly_branch_performance, table_name="monthly_branch_performance", order_by_cols="branch_id", mode="overwrite")
