...
from finance_performance.sql import process_finance_performance
...
def main():
    ...
    print("Execution finance_performance")
    finance_performance = process_finance_performance(spark)
    finance_performance.printSchema()
    finance_performance.show(truncate=False, n=20)

    writer = ch_writer()
    writer.write_to_clickhouse(finance_performance, table_name="finance_performance", order_by_cols="unit_sold", mode="overwrite")
