...
from monthly_finance_performance.sql import process_monthly_finance_performance
...
def main():
    spark = SparkSession.builder \
        .appName("monthly_finance_performance") \
        .getOrCreate()
    ...
    print("Execution monthly_finance_performance")
    monthly_finance_performance = process_monthly_finance_performance(spark)
    monthly_finance_performance.printSchema()
    monthly_finance_performance.show(truncate=False, n=20)

    writer = ch_writer()
    writer.write_to_clickhouse(monthly_finance_performance, table_name="monthly_finance_performance", order_by_cols="month", mode="overwrite")
