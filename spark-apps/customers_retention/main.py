...
from customers_retention.sql import process_customers_retention
...
def main():
    ...
    print("Execution customers_retention")
    customers_retention = process_customers_retention(spark)
    customers_retention.printSchema()
    customers_retention.show(truncate=False, n=20)

    writer = ch_writer()
    writer.write_to_clickhouse(customers_retention, table_name="customers_retention", order_by_cols="bulan", mode="overwrite")
