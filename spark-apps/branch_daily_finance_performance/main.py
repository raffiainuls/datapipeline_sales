...
from branch_daily_finance_performance.sql import process_branch_daily_finance_performance
...
def main():
    ...
    print("Execution branch_daily_finance_performance")
    branch_daily_finance_performance = process_branch_daily_finance_performance(spark)
    branch_daily_finance_performance.printSchema()
    branch_daily_finance_performance.show(truncate=False, n=20)

    writer = ch_writer()
    writer.write_to_clickhouse(branch_daily_finance_performance, table_name="branch_daily_finance_performance", order_by_cols="name, date", mode="overwrite")
