...
from daily_finance_performance.sql import process_daily_finance_performance
...
def main():
    spark = SparkSession.builder \
        .appName("daily_finance_performance") \
        .getOrCreate()
    ...
    print("Execution daily_finance_performance")
    daily_finance_performance = process_daily_finance_performance(spark)
    daily_finance_performance.printSchema()
    daily_finance_performance.show(truncate=False, n=20)

    writer = ch_writer()
    writer.write_to_clickhouse(daily_finance_performance, table_name="daily_finance_performance", order_by_cols="date", mode="overwrite")
